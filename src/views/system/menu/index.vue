    ...
    >
      ...
      <el-table-column prop="icon" label="图标" align="center" width="100">
        <template #default="scope">
          <el-icon v-if="['M', 'C'].indexOf(scope.row.menuType) >= 0" :key="scope.row.id + 'icon1'">
            <component :is="scope.row.icon" />
          </el-icon>
          <span v-else :key="scope.row.id + 'icon2'">
            {{ scope.row.icon }}
          </span>
        </template>
      </el-table-column>
...
import { addMenu, delMenu, getMenu, listMenu, updateMenu, type MenuObj } from "@/api/system/menu";
...
/** 修改按钮操作 */
async function handleUpdate(row: MenuObj) {
  reset();
  await getTreeselect();
