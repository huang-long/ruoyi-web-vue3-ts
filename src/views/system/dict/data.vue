    ...
    <el-form v-show="showSearch" ref="queryRef" :model="queryParams" :inline="true">
      ...
      <el-form-item label="状态" prop="status">
        <el-select v-model="queryParams.status" placeholder="数据状态" clearable style="width: 200px">
          <el-option v-for="item in dict.sys_normal_disable" :key="item.value" :label="item.label" :value="item.value" />
        </el-select>
      </el-form-item>
      <el-form-item>
      ...
      <el-form ref="dataRef" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="状态" prop="status">
          <el-radio-group v-model="form.status">
            <el-radio v-for="item in dict.sys_normal_disable" :key="item.value" :label="item.value">{{ item.label }}</el-radio>
          </el-radio-group>
        </el-form-item>
...
const dict = loadDicts("sys_normal_disable");

const dataList = ref<DictObj[]>([]);
...
const typeOptions = ref<DictTypeObj[]>([]);
  ...
  dataRef.value?.validate((valid) => {
    if (valid) {
      ...
      subPromiss.then(() => {
        ...
        ElMessage.success(addFlag ? "新增成功" : "修改成功");
        open.value = false;
        getList();
      });
    }
...
// 加载数据
if (typeof route.params.dictId === "string") {
