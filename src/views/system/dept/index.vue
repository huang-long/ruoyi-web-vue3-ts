...
import { listDept, getDept, delDept, addDept, updateDept, listDeptExcludeChild, type DeptObj } from "@/api/system/dept";
...
function reset() {
  ...
  deptRef.value && deptRef.value.resetFields();
}
/** 搜索按钮操作 */
...
/** 新增按钮操作 */
function handleAdd(row: DeptObj) {
  reset();
  listDept().then((response) => {
    response.data && (deptOptions.value = handleTree(response.data, "deptId"));
  });
  if (row != undefined) {
