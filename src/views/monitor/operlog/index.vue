    ...
    <el-form v-show="showSearch" ref="queryRef" :model="queryParams" :inline="true" label-width="68px">
      ...
      <el-form-item label="类型" prop="businessType">
        <el-select v-model="queryParams.businessType" placeholder="操作类型" clearable style="width: 240px">
          <el-option v-for="dict in dicts.sys_oper_type" :key="dict.value" :label="dict.label" :value="dict.value" />
        </el-select>
      </el-form-item>
    ...
    <el-table ref="operlogRef" v-loading="loading" :data="operlogList" :default-sort="defaultSort" @selection-change="handleSelectionChange" @sort-change="handleSortChange">
      ...
      <el-table-column label="操作类型" align="center" prop="businessType">
        <template #default="scope">
          <dict-tag :options="dicts.sys_oper_type" :value="scope.row.businessType" />
        </template>
      </el-table-column>
    ...
    <el-dialog v-model="open" title="操作日志详细" width="800px" append-to-body>
      <el-form :model="form" label-width="100px">
        <el-row>
          ...
          <el-col :span="8">
            <el-form-item label="操作时间：">{{ form.operTime ? dayjs(form.operTime).format("YYYY-MM-DD HH:mm:ss") : "" }}</el-form-item>
          </el-col>
          <el-col :span="24">
...
<script lang="ts" setup name="OperlogPage">
...
const operlogList = ref<OperLogObj[]>([]);
...
const queryParams = ref<OperLogObj & QueryParam>({
  pageNum: 1,
  pageSize: 10,
  operIp: undefined,
  title: undefined,
