    ...
    <el-form v-show="showSearch" ref="queryRef" :model="queryParams" :inline="true">
      ...
      <el-form-item label="任务状态" prop="status">
        <el-select v-model="queryParams.status" placeholder="请选择任务状态" clearable style="width: 200px">
          <el-option v-for="dict in dicts.sys_job_status" :key="dict.value" :label="dict.label" :value="dict.value" />
        </el-select>
      </el-form-item>
      <el-form-item>
          ...
          <el-col :span="12">
            <el-form-item label="任务分组" prop="jobGroup">
              <el-select v-model="form.jobGroup" placeholder="请选择">
                <el-option v-for="dict in dicts.sys_job_group" :key="dict.value" :label="dict.label" :value="dict.value"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="24">
          ...
          <el-col v-if="form.jobId !== undefined" :span="24">
            <el-form-item label="状态">
              <el-radio-group v-model="form.status">
                <el-radio v-for="dict in dicts.sys_job_status" :key="dict.value" :label="dict.value">{{ dict.label }}</el-radio>
              </el-radio-group>
            </el-form-item>
          </el-col>
          <el-col :span="12">
        ...
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="任务分组：">
              <dict-tag :options="dicts.sys_job_group" :value="form.jobGroup" />
            </el-form-item>
          ...
          <el-col :span="12">
            <el-form-item label="下次执行时间：">
              {{ dayjs(form.nextValidTime).format("YYYY-MM-DD HH:mm:ss") }}
            </el-form-item>
          </el-col>
          <el-col :span="24">
...
<script lang="ts" setup name="JobPage">
import server from "@/utils/request";
import { listJob, getJob, delJob, addJob, updateJob, runJob, changeJobStatus, type JobObj } from "@/api/monitor/job";
...
// 更多操作触发
// function handleCommand(command: string, row: JobObj) {
//   switch (command) {
//     case "handleRun":
//       handleRun(row);
//       break;
...
function handleStatusChange(row: JobObj) {
  ...
  ElMessageBox.confirm('确认要"' + text + '""' + row.jobName + '"任务吗?')
    .then(function () {
      return changeJobStatus(row.jobId, row.status || "0");
    })
    .then(() => {
...
function handleRun(row: JobObj) {
  ElMessageBox.confirm('确认要立即执行一次"' + row.jobName + '"任务吗?')
    .then(function () {
      return runJob(row.jobId, row.jobGroup || "0");
    })
    .then(() => {
