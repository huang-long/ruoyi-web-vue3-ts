...
<script lang="ts" setup name="CompCrontabMonth">
...
function changeRadioValue(value: string) {
  if (value === "*") {
    ...
    cycle01.value = props.check(Number(indexArr[0]), 1, 11);
    cycle02.value = props.check(Number(indexArr[1]), cycle01.value + 1, 12);
    radioValue.value = 2;
  } else if (value.indexOf("/") > -1) {
    const indexArr = value.split("/");
    average01.value = props.check(Number(indexArr[0]), 1, 11);
    average02.value = props.check(Number(indexArr[1]), 1, 12 - average01.value);
    radioValue.value = 3;
  } else {
